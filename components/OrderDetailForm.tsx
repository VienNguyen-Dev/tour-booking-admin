...
import EditRedeemForm from "./EditRedeemForm";

declare type OrderDetailFormProps = {
  order: Order;
  pageType: string;
};
const OrderDetailForm = ({ order, pageType }: OrderDetailFormProps) => {
  ...
  const handleArchive = async () => {
    if (voucherRef.current) {
      ...
      linkElement.download = `order_${orderId}.png`;
          ...
          <div className="flex gap-4 justify-end">
            {pageType === "order" ? (
              <div className="flex gap-4">
                <Button onClick={handleExchange} className="flex gap-1 h-10 py-2 px-5 rounded-[4px]  border border-[#0D062D1A] cursor-pointer text-sm font-medium items-center hover:bg-orange-200">
                  <Image src={"/assets/icons/exchange.png"} width={20} height={20} alt="archive" />
                  Exchange
                </Button>
                <Button onClick={handleRefund} className="flex gap-1 h-10 py-2 px-5 rounded-[4px]  border border-[#0D062D1A] cursor-pointer text-sm font-medium items-center hover:bg-orange-200">
                  <Image src={"/assets/icons/Undo.png"} width={20} height={20} alt="archive" />
                  Refund
                </Button>
                <Button onClick={handleCancel} className="flex gap-1 h-10 py-2 px-5 rounded-[4px]  border border-[#0D062D1A] cursor-pointer text-sm font-medium items-center hover:bg-orange-200">
